...
public class InsertHtmlIntoWordAtPlaceholder {
...
//            String periodText = "<p>本协议期限为<span class=\"mention\" data-index=\"0\" data-denotation-char=\"@\" data-id=\"1\" data-value=\"{dateRange}\">\uFEFF<span contenteditable=\"false\">@{dateRange}</span>\uFEFF</span> ，为期<span class=\"mention\" data-index=\"1\" data-denotation-char=\"@\" data-id=\"2\" data-value=\"{month}\">\uFEFF<span contenteditable=\"false\">@{month}</span>\uFEFF</span> 。期满时除经双方协商一致延长外，本协议自动终止，但双方应按照第十二条第4款约定处理后续事宜。</p>";
            ...
            periodText = periodText.replaceAll("&nbsp;"," ");
            Document doc = Jsoup.parse(periodText);

            // 创建 XHTMLImporterImpl 实例
            XHTMLImporterImpl xhtmlImporter = new XHTMLImporterImpl(wordMLPackage);

            // 将 HTML 内容转换为 Word 中的对象
            List<Object> contentList = xhtmlImporter.convert(doc.html(),null);
            ...
            boolean replaced = replacePlaceholderWithContent(mainDocumentPart, placeholder, contentList);

            if (replaced){
                // 保存修改后的文档
                wordMLPackage.save(new File("D:\\Development\\BuisFile\\testfile\\销售模块-TCS协议打印 - 副本insert3.docx"));
    ...
    private static boolean replacePlaceholderWithContent(MainDocumentPart mainDocumentPart, String placeholder, List<Object> htmlContent) {
        ...
        for (Object paragraph : paragraphs) {
            ...
            for (Object text : texts) {
                ...
                if (placeholder.equals(textElement.getValue())) {
                    paragraphs.remove(index);
                    paragraphs.addAll(index,htmlContent);
                    replaced = true;
                    break;
                }
            }
